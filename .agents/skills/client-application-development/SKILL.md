...
Stop if the project prefix, canonical version owner, signing authority, confirmation boundary, active-work drain condition, or post-activation recovery contract is missing and cannot be discovered safely. Missing compatibility policy uses the no-historical-compatibility default above; a claimed in-place promise must come from the ProductContract. A local installer is not a published release, and an OSS outage does not authorize a fallback origin.
