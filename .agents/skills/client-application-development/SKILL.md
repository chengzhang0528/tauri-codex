...
Before implementation, identify the client variant, current technology stack, supported OS/device/browser and architectures, installation source, OSS project prefix, canonical version source, installer/package boundary, update owner, component owners, user confirmation boundary, active-work drain rule, post-activation recovery contract, hidden-process requirement, signing/provenance requirement, and this request's execution type. Resolve each project fact from the formal owner and code evidence. If owners conflict, repair the single fact owner before expanding scope.
...
- Before freezing manifest or candidate identity, run the cheapest final runtime checks against the signed component trees that will actually be archived or installed, such as a Manager runtime check and bundled CLI `--version`. Dependency preparation or cache-hit checks do not replace this final candidate doctor.
...
- A launcher, updater, source-policy, or bootstrap-compatibility change requires a new Installer/Launcher version. Do not infer an in-place compatibility bridge merely because a Launcher or updater exists. Require one only when the ProductContract promises in-place compatibility or Setup/reinstall cannot preserve user-owned state. Otherwise stop before incompatible installation mutation, report that the newer official Setup is required, use uninstall/reinstall only as a fallback, and preserve user data. Do not add dual protocols, dual schemas, compound upgrade closures, rollback machinery, or historical-state migration solely to avoid reinstall.
...
- Treat compatibility as an owned product promise, not a default hardening checklist. For example, a low-frequency Launcher protocol break with no in-place promise and user state outside the install root should select Setup/reinstall, not a bridge. If the product explicitly promises in-place upgrades or reinstall cannot preserve required state, own the necessary bridge or migration instead of using reinstall to evade that contract.
...
Stop if the project prefix, canonical version owner, signing authority, compatibility rule, confirmation boundary, active-work drain condition, or post-activation recovery contract is missing and cannot be discovered safely. A local installer is not a published release, and an OSS outage does not authorize a fallback origin.
