...
- For thin installers, additionally verify: blank machine bootstraps successfully; eligible system components are shown as reused; missing components are fetched from the fixed source; digest/doctor failures keep the old current release; repeat MSI execution preserves registration and user data; activation follows the selected post-activation recovery contract.
