...
`.github/workflows/ci.yml` 在 Pull Request 和 `main` 推送时安装锁定依赖，执行依赖审计和 `npm test`。self-use 候选只通过 GitHub Actions 页面手工运行 `.github/workflows/windows-release.yml`：选择 `candidate`，填写版本 `0.2.0` 和准备发布的完整 40 位 source commit。该操作不需要 Authenticode PFX 或 Ed25519 发布密钥，只构建并复核一次，保留 14 天的 frozen candidate artifact；它不写 OSS、不创建 tag，也不创建 GitHub Release。
